...
const Desert = () => {
  ...
  useEffect(() => {
    const fetchDesert = async () => {
      try {
        const snapshot = await firestore().collection('Desert').get();
        ...
        setDesertItems(items);
        setFilteredItems(items);
      } catch (error) {
        console.error("Error fetching Desert items:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSearch = (query) => {
    ...
    if (query) {
      const filteredData = DesertItems.filter(item => 
        item.Foodname.toLowerCase().includes(query.toLowerCase())
      );
      setFilteredItems(filteredData);
    } else {
      setFilteredItems(DesertItems);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Desert Menu</Text>
      ...
      {filteredItems.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={item => item.item.empty ? renderPlaceholder() : renderDesertItem(item)}
