...
const Rice = () => {
  ...
  useEffect(() => {
    const fetchRice = async () => {
      try {
        const snapshot = await firestore().collection('Rice').get();
  ...
  const handleSearch = (query) => {
    ...
    if (query) {
      const filteredData = RiceItems.filter(item => 
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Rice Menu</Text>
      ...
      {filteredItems.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={item => item.item.empty ? renderPlaceholder() : renderRiceItem(item)}
