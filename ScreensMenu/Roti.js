...
const Roti = () => {
  ...
  useEffect(() => {
    const fetchRoti = async () => {
      try {
        const snapshot = await firestore().collection('Roti').get();
        ...
        console.error("Error fetching Roti items:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSearch = (query) => {
    ...
    if (query) {
      const filteredData = RotiItems.filter(item => 
        item.Foodname.toLowerCase().includes(query.toLowerCase())
      );
      setFilteredItems(filteredData);
    } else {
      setFilteredItems(RotiItems);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Roti Menu</Text>
      ...
      {filteredItems.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={item => item.item.empty ? renderPlaceholder() : renderRotiItem(item)}
